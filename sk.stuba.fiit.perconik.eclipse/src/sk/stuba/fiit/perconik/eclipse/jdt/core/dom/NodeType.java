 ...
 * @since 1.0
 */
public enum NodeType implements IntegralConstant, TypeConstant<ASTNode> {
  /**
  ...
  private static final IntegralConstantSupport<NodeType> integers = IntegralConstantSupport.of(NodeType.class);

  private static final TypeConstantSupport<NodeType, ASTNode> types = TypeConstantSupport.of(NodeType.class);
  ...
  private NodeType(final int value, final Class<? extends ASTNode> type) {
    assert type != null;
