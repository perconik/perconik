...
abstract class AbstractRegistrationPreferences<R extends Registration> extends AbstractObjectPreferences {
  ...
  final void setRegistrations(final String key, final Set<R> registrations) {
    try {
      ...
      PluginConsoles.create(Activator.defaultInstance()).error(e, "Unable to write registrations under key %s", key);
    }
  }
  ...
  final Set<R> getRegistrations(final String key) {
    try {
      ...
      PluginConsoles.create(Activator.defaultInstance()).error(e, "Unable to read registrations under key %s", key);
      ...
      if (this.scope() != Scope.DEFAULT) {
        ...
        try {
          ...
          PluginConsoles.create(Activator.defaultInstance()).error(e, "Unable to synchronize registrations under key %s", key);
        }
